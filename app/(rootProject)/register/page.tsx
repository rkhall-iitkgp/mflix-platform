...
export default function Register(props: any) {
    ...
    const handleRegister = async (values: any) => {
        ...
        // setUserData(values);
        console.log(values);
        setFormData({ ...values, type: "register" });
        const userData = {
            email: values.email,
            type: "register",
        };
    ...
    const form = useForm({
        ...
        validate: {
            ...
            phone: (val) =>
                val.toString().length < 10 ? "Invalid Phone No." : null,
            email: (val) => (/^\S+@\S+$/.test(val) ? null : "Invalid email"),
            newPassword: (val) =>
                val.length <= 5
                    ? "Password should include at least 6 characters"
                    : null,
            confPassword: (val, values) =>
                val != values.newPassword ? "Password does not match" : null,
        },
    });
