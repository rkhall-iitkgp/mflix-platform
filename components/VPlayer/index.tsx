...
const VideoPlayer = forwardRef<HTMLVideoElement, Props>(({ ws }: Props, ref) => {
  ...
  const seekPlayerToLocation = (e: any) => {
    if (progressBarRef.current && progressRef.current) {
      ...
      const x = e.clientX - progressBarRef.current.getBoundingClientRect().left;
  ...
  const setCurrVolume = (e: any) => {
    if (volumeBarRef.current && volumeRef.current) {
      ...
      const x = e.clientX - volumeBarRef.current.getBoundingClientRect().left;
      let percentage = (x / width) * 100;
      if (percentage < 0) percentage = 0;
      if (percentage > 100) percentage = 100;
  ...
  const updateProgress = () => {
    if (!playerRef.current || !progressRef.current || !currTimeRef.current || !totalTimeRef.current)
      return;
    storeSetCurrentTime(playerRef.current?.currentTime);
  ...
  useEffect(() => {
    ...
    const mouseMove = (e: any) => {
      if (isSeeking && progressBarRef.current && progressRef.current) {
        const { width } = progressBarRef.current.getBoundingClientRect();
        const x = e.clientX - progressBarRef.current.getBoundingClientRect().left;
        let percentage = (x / width) * 100;
        if (percentage < 0) percentage = 0;
    ...
    return () => {
      if (dotRef.current) dotRef.current!.removeEventListener('mousemove', mouseMove);
  ...
  useEffect(() => {
    ...
    const mouseMove = (e: any) => {
      if (isVolumeSeeking && volumeBarRef.current && volumeRef.current) {
        const { width } = volumeBarRef.current.getBoundingClientRect();
        const x = e.clientX - volumeBarRef.current.getBoundingClientRect().left;
        let percentage = (x / width) * 100;
        if (percentage < 0) percentage = 0;
    ...
    return () => {
      volumeDotRef.current?.removeEventListener('mousemove', mouseMove);
      document.removeEventListener('mousemove', mouseMove);
      document.removeEventListener('mouseup', mouseUp);
    };
  }, [isVolumeSeeking]);
  ...
  useEffect(() => {
    ...
    return () => {
      playerContainerRef.current?.removeEventListener('click', handleClick);
    };
  }, []);
    ...
    <div ref={playerContainerRef} className={style.videoContainer}>
      ...
      <div ref={controlsRef} className={style.controlsContainer}>
        <div className={style.progressContainer}>
          ...
          >
            <div id="progress" ref={progressRef} className={style.progress}></div>
            <div id="dot" ref={dotRef} className={style.dot}></div>
          </div>
        ...
        <div className={style.controls}>
          <div className={style.left_controls}>
            ...
            >
              ...
              >
                <div ref={volumeBarRef} onClick={setCurrVolume} className={style.volBar}>
                  <div ref={volumeRef} className={style.volume}></div>
                  <div ref={volumeDotRef} className={style.volumeDot}></div>
                </div>
              </div>
            </div>
          </div>
          <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
            ...
            <button onClick={() => toggleChat(undefined)} className={style.partyWatch}>
              <Image
                src={PartyWatchIcon}
                alt="PartyWatchIcon"
                width={25}
